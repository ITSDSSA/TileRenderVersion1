...
namespace Path_AI
{
    public static class PathFinder
    {
        ...
        public static void GenerateMaze(List<Tile> Visited, Stack<Tile> frontier, List<Tile> tiles, Tile Current)
        {
            TileComparer compare = new TileComparer();
            List<Tile> nonVisitedNeighbours = adjacentTiles(tiles, Current)
                .Where(tile => tile.Passable == false && !Visited.Contains(tile,compare))
                .ToList();
            List<Tile> VisitedNeighbours = adjacentTiles(tiles, Current).Where(tile => tile.Passable == true).ToList();
            if (nonVisitedNeighbours.Count > 0)
            {
                Current = nonVisitedNeighbours[new Random().Next(0, nonVisitedNeighbours.Count - 1)];
                Current.Passable = true;
                Visited.Add(Current);
                frontier.Push(Current);
                GenerateMaze(Visited,frontier, tiles, Current);
            }
        ...
        public static List<Tile> adjacentTiles(List<Tile> tiles, Tile t)
        {
            // above
            // width
            int Width = tiles.Select(tile => tile.X).Max();
            // Height
            int Height = tiles.Select(tile => tile.Y).Max();

            var neighbours = tiles.Where(tile =>
                                        (tile.X > -1 && tile.X == t.X - 1 && tile.Y == t.Y) // Left
                                        || (tile.X <= Width && tile.X == t.X + 1 && tile.Y == t.Y) // right
                                        ||(tile.Y > -1 && tile.Y == t.Y - 1 && tile.X == t.X) // up
                                        || (tile.Y <= Height && tile.Y == t.Y + 1 && tile.X == t.X) // down
                                        ).ToList();
